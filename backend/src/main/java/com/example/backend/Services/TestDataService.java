...
public class TestDataService {
    ...
    @Transactional (rollbackFor = TestDataInsertionException.class)
    public void insertTestData(TestData testData) throws TestDataInsertionException {

        testDataRepository.insertTestData(testData);

        // Data in user profile is ALWAYS updated after each test -> done in a separate method already.
        ...
        if (optionalPersonalRecord.isPresent()) { // Personal Record for that user and test type exists
            ...
            if (testData.getWordsPerMinute() > personalRecord.getWordsPerMinute()) { // New test data has higher wpm score
                ...
                personalRecord.setAccuracy(testData.getAccuracy());
